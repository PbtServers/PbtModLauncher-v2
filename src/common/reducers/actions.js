...
export function processModrinthManifest(instanceName) {
  return async (dispatch, getState) => {
    ...
    dispatch(updateDownloadStatus(instanceName, 'Descargando Pack...'));
    ...
    dispatch(updateDownloadStatus(instanceName, 'Finalizando Archivos...'));
  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (isUpdate && !bypassCopy) {
        dispatch(
          updateDownloadStatus(instanceName, 'Creando Punto de Restauración...')
        );
    ...
    switch (instance.loader.source) {
      ...
      case MODRINTH: {
        ...
        switch (loaderType) {
          ...
          default:
            throw Error(
              `Esta instancia (${instanceName}) requiere un cargador no soportado: ${loaderType}`
            );
        }
...
export const checkForPortableUpdates = () => {
  return async (dispatch, getState) => {
    ...
    if (newVersion) {
      const baseAssetUrl = `https://github.com/PbtServers/PbtModLauncher-v2/releases/download/${newVersion?.tag_name}`;
        ...
        async file => {
          ...
          try {
            ...
            try {
              ...
              if (!sha1Matched) {
                throw new Error(`No se puede descargar ${file.compressedSha1}`);
              }
